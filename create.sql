...
CREATE TABLE TimeStampp (
    time_acc VARCHAR(60) NOT NULL REFERENCES TimeAcc(time_acc),
    date_acc VARCHAR(60) NOT NULL REFERENCES DateAcc(date_acc),
    TimeStamp_id INT NOT NULL UNIQUE,
    CONSTRAINT PK_timestamp PRIMARY KEY (time_acc, date_acc)
    );
--entity Address_acc
CREATE TABLE Address_acc (
    ...
    CONSTRAINT PK_address_acc PRIMARY KEY (address, town)
    );
--entity Accident
